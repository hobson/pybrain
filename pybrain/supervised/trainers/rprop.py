...
class RPropMinusTrainer(BackpropTrainer):
    ...
    (possibly sequential) by RProp without weight backtracking (aka RProp-,
    cf. [Igel&Huesken, Neurocomputing 50, 2003]) and without ponderation,
    ie. all training samples have the same weight."""
    ...
    def train(self):
        ...
        for seq in self.ds._provideSequences():
            ...
            ponderation += p
        if self.verbose:
            print(("epoch {epoch:6d}  total error {error:12.5g}   "
                   "avg weight  {weight:12.5g}".format(
                       epoch=self.epoch, error=errors / ponderation,
